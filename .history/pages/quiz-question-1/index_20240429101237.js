...
export default function QuestionOne() {
  ...
  return (
    <>
      <HeaderArea title="Quiz questions" description="Quiz question #1"/>
      <div className={`${styles.question_1} ${styles.quizNav}`}> 
        <nav className={styles.quizQuestionNav}>
          <Link href="/quiz-intro">
            <Image src='/images/quiz/back-arrow.png' alt="back arrow" width={62} height={106} tabIndex={1}/>
          </Link>
          ...
          <div className={styles.quiz}>      
            ...
            <div className={styles.questions}>
              ...
              <label htmlFor="a" className={selectedValues.question_1 === 'A' ? styles.selected : ''} tabIndex={2}>A. Red, Green, Blue</label>
              <input type="radio" id="b" name="question_1" value="B" onChange={handleRadioChange} />
              <label htmlFor="b" className={selectedValues.question_1 === 'B' ? styles.selected : ''} tabIndex={3}>B. Cyan, Magenta, Yellow</label>
              <input type="radio" id="c" name="question_1" value="C" onChange={handleRadioChange} />
              <label htmlFor="c" className={selectedValues.question_1 === 'C' ? styles.selected : ''} tabIndex={4}>C. Red, Yellow, Blue</label>  
              <input type="radio" id="d" name="question_1" value="D" onChange={handleRadioChange} />
              <label htmlFor="d" className={selectedValues.question_1 === 'D' ? styles.selected : ''} tabIndex={5}>D. Green, Yellow, Violet</label>  
              <button className={styles.button} onClick={() => {selectedValues.question_1 && setAnswerSubmitted(true)}} tabIndex={6}>Next</button>        
            </div>
          </div>      
      </div>
    </>
